...
//  vec4 objectColor = mix(texture(texture1, uvPos), texture(texture2, uvPos), 1.0);
  ...
  vec3 lightDirN = normalize(worldPosition - lightPosition); // 光线方向
  vec3 viewDir = normalize(worldPosition - cameraPosition); // 求视线方向
  vec3 targetDirectionN = normalize(targetDirection); // 视线方向

  // 计算照射范围
  float theta = dot(lightDirN, targetDirectionN);
  float visible = cos(visibleAngle);
  float spotFlag = step(visible, theta);
